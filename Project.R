...
fname1 <- file.choose()
fname1
df1 <- read.csv(fname1, header=T, stringsAsFactors = F)

resptime <- df1 %>% select(2,3,4,9,17)
...
l <-ggplot(top, aes(x=City, y=AvgResTime)) + geom_bar(stat = "identity", color="black", fill="red")+ ggtitle("Slowest Response Times")

ggplot(fastest, aes(x=City, y=AvgResTime)) + geom_point()+  ylim(300,425)
p<- ggplot(fastest, aes(x=City, y=AvgResTime)) + geom_bar(stat = "identity", color="black", fill="darkgreen")+ggtitle("Fastest Response Times")+ ylim(0,425)

require(gridExtra)
grid.arrange(l, p, ncol=2)
